...
def get_topics(topic_word_dist: torch.Tensor, top_k: int, id2token: Mapping[int, str]):
    word_dist = torch.softmax(topic_word_dist, dim=-1)
    _, indices = torch.topk(word_dist, top_k, dim=-1)
...
class ProdLDA(LightningModule):
    def __init__(self, id2token=None, encoder_hidden_dim=100, topic_num=50, dropout=0.2, affine=False, alpha=None,
                 metric='npmi'):
        super().__init__()
    ...
    def forward(self, x_bow):
        return self.model.encoder(x_bow)
